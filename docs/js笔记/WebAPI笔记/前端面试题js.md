...
```html
 <style>
     .box{
         ...
         text-overflow: ellipsis;/*文字溢出隐藏显示样式*/
...
![image-20191027161719084](assets/image-20191027161719084.png)

### 多行文本溢出隐藏

> 但有时候就需要显示稍微多一点文字  但也需要隐藏,这样就可以用上多行文本溢出隐藏了
...
![image-20191027163324638](assets/image-20191027163324638.png)

##  iframe标签

* 优点
  * 便于修改,方便管理, 模块分离 一些类似管理系统中用的到
* 缺点
  * 加载速度慢, 比dom元素创建慢了1-2个数量级
  * iframe标签会造成页面的阻塞,不能很好的触发onload事件 会让用户感觉到页面加载很慢,用户体验不好 
  * 对ESO 的不好, 搜索引擎无法吧关键字检测到的优先级归属到iframe标签的网站,

## 闭包原理

> 官方解释 :
>
> 一个拥有许多变量和绑定了这些变量的环境的表达式(通常为一个函数), 因而这些变量也是该表达式的一部分

### 闭包的概念

**闭包就是能够读取到其他函数内部变量的函数**

由于在javascript 语言中,只有函数内部的子函数才能够读取父级函数的局部变量,因此可以吧闭包简单理解成`定义在一个函数内部的函数`

所以本质上,闭包就是将函数内部与函数外部链接起来的做桥梁

### 闭包的用途

闭包可以用在许多地方,它的最大的用处有两个:

1. 一个是通过在需要提取函数变量的内部中定义一个新的函数,通过吧新的函数return出去,从而得到 函数内部的变量
2. 让这些变量始终保持在内存中
